...
export class SessionRecorderService {
  ...
  async importSession(file: File): Promise<RecordedSession | null> {
    try {
      ...
      if (session.version !== 1 || !session.metadata || !Array.isArray(session.events)) {
        this.log.error('SessionRecorder', 'Invalid session file format');
        return null;
      }

      return session;
    } catch (e) {
      this.log.error('SessionRecorder', 'Failed to parse session file:', e);
      return null;
    }
  }
  ...
  saveToLocalStorage(session: RecordedSession): void {
    try {
      ...
      this.log.warn('SessionRecorder', 'Failed to save session to localStorage:', e);
    }
  }
  ...
  loadSavedSessions(): SessionMetadata[] {
    ...
    try {
      ...
      this.log.warn('SessionRecorder', 'Failed to load sessions from localStorage:', e);
    }
    return sessions;
  }
  ...
  loadFromLocalStorage(recordedAt: string): RecordedSession | null {
    try {
      ...
      this.log.warn('SessionRecorder', 'Failed to load session from localStorage:', e);
    }
    return null;
  }
