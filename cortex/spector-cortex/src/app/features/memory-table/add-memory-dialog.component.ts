  ...
  template: `
    ...
    <mat-dialog-content class="dialog-content">
      ...
      @if (mode === 'ingest') {
        ...
        <mat-button-toggle-group
          [(ngModel)]="ingestMethod"
          class="ingest-method-toggle"
          appearance="standard"
        >
          ...
          >
            ...
            @if (selectedFiles.length > 0) {
              ...
              <span class="upload-hint"
                >Supports text files: .txt, .md, .java, .py, .js, .ts, .json, etc.</span
              >
        ...
        <!-- ── Folder Upload Sub-section ── -->
        @if (ingestMethod === 'folder') {
          <div
            class="file-upload-zone"
            [class.drag-over]="isDragOver"
          ...
          >
            ...
            />
            @if (selectedFiles.length > 0) {
              <mat-icon class="upload-icon uploaded">folder</mat-icon>
              <span class="upload-label">{{ folderName }} — {{ selectedFiles.length }} file(s)</span>
              <span class="upload-hint">{{ formatTotalSize() }}</span>
              <button
                mat-stroked-button
                class="add-more-btn"
                (click)="folderInput.click(); $event.stopPropagation()"
              >
                <mat-icon>sync</mat-icon> Change folder
              </button>
            } @else {
              <mat-icon class="upload-icon">drive_folder_upload</mat-icon>
              <span class="upload-label">Click to select a folder</span>
              <span class="upload-hint">All text files in the folder and subfolders will be ingested</span>
            }
          </div>
        }
      }
      ...
      @if (mode === 'text') {
        <div class="slider-section">
          ...
          <div class="slider-row">
            ...
            <span
              class="slider-value cortex-mono"
    ...
    <mat-dialog-actions align="end" class="dialog-actions">
      ...
      >
        <mat-icon>{{
          mode === 'text' ? 'memory' : ingestMethod === 'file' ? 'upload' : 'drive_folder_upload'
