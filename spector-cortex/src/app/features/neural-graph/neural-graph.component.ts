...
const MAX_NODES = 200;
const MAX_PARTICLES = 40;  // Hard cap to prevent OOM
...
export class NeuralGraphComponent implements AfterViewInit, OnDestroy {
  ...
  constructor() {
    ...
    effect(() => {
      ...
      if (trace && this.scene) {
        ...
        this.launchTraversalParticles(
          trace.hebbianActivated + trace.temporalLinked + trace.entityDiscovered,
        );
      }
    });
    ...
    effect(() => {
      ...
      if (diag && this.scene && !this.nodesInitialized) {
        const total =
          diag.workingCount + diag.episodicCount + diag.semanticCount + diag.proceduralCount;
        if (total > 0) {
  ...
  ngAfterViewInit(): void {
    ...
    // Check if memory diagnostic data is already available in signal
    const diag = this.state.memoryDiag();
    if (diag && !this.nodesInitialized) {
      const total =
        diag.workingCount + diag.episodicCount + diag.semanticCount + diag.proceduralCount;
      if (total > 0) {
        this.nodesInitialized = true;
    ...
    // Fallback: if no SSE data yet, fetch from REST API
    if (!this.nodesInitialized) {
      this.memoryService.getStatus().subscribe({
        next: (status) => {
          if (this.nodesInitialized) return; // SSE arrived in the meantime
          const total = Object.values(status.tierCounts).reduce((a, b) => a + b, 0);
          if (total > 0) {
            this.nodesInitialized = true;
            this.generateNodes(Math.min(MAX_NODES, Math.max(20, total)));
            this.generateEdges();
        ...
        error: () => {
          // Backend unreachable — will populate when SSE/mock starts
        },
      });
    }

    // Apply active cognitive profile colors
    this.applyProfileVisuals(this.state.activeProfile());
  ...
  onWheel(event: WheelEvent): void {
    this.orbitRadius = Math.max(20, Math.min(200, this.orbitRadius + event.deltaY * 0.05));
    event.preventDefault();
  }
      ...
      for (let j = i + 1; j < this.nodes.length && connections < 3; j++) {
        ...
        if (dist < 20 && Math.random() > 0.55) {
          ...
          const material = new THREE.LineBasicMaterial({
            color,
            transparent: true,
            opacity: type === 'hebbian' ? 0.08 : 0.06,
          });
  ...
  private launchTraversalParticles(count: number): void {
    // Launch a burst of particles — capped to prevent OOM
    const burstSize = Math.min(MAX_PARTICLES - this.particles.length, Math.max(10, count * 3));
  ...
  private animate(): void {
    ...
    for (const edge of this.graphEdges) {
      const visible =
        (edge.type === 'hebbian' && layers.hebbian) ||
    ...
    if (layers.particles) {
      ...
      if (this.ambientParticleTimer > 0.15 && this.particles.length < MAX_PARTICLES) {
    ...
    if (layers.particles) {
      for (const particle of this.particles) {
        ...
        if (particle.trailMesh) {
          ...
          particle.trailMesh.position.lerpVectors(
            fromNode.position,
            toNode.position,
            trailProgress,
          );
      ...
      // Clean up dead particles — DISPOSE geometries/materials to prevent OOM
      for (const particle of this.particles) {
        if (!particle.alive) {
          ...
          if (particle.trailMesh) {
            ...
            (particle.trailMesh.material as THREE.Material).dispose();
          }
        }
      }
