...
export class VectorSpaceComponent implements AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit(): void {
    ...
    // Check if query vector is already available in the signal
    const qv = this.state.queryVector();
    if (qv && this.queryDot) {
      this.queryDot.position.set(qv[0], qv[1], qv[2]);
      (this.queryDot.material as THREE.MeshBasicMaterial).opacity = 1;
  ...
  onWheel(event: WheelEvent): void {
    this.orbitRadius = Math.max(15, Math.min(200, this.orbitRadius + event.deltaY * 0.05));
    event.preventDefault();
  }
  ...
  private buildAxesGrid(): void {
    ...
    const xLine = new THREE.Line(
      xGeo,
      new THREE.LineBasicMaterial({ color: 0xff6666, transparent: true, opacity: 0.3 }),
    );
    ...
    const yLine = new THREE.Line(
      yGeo,
      new THREE.LineBasicMaterial({ color: 0x66ff66, transparent: true, opacity: 0.3 }),
    );
  ...
  /** Generate 3D point cloud from memory rows using hash-based projection */
  private generatePointsFromRows(rows: MemoryRow[]): void {
    const tierClusters: Record<string, [number, number, number]> = {
      WORKING:    [0, 0, 0],
      EPISODIC:   [18, -12, 8],
      SEMANTIC:   [-15, 20, -10],
      PROCEDURAL: [10, 10, -20],
    ...
    const points = rows.map((row, i) => {
      const center = tierClusters[row.tier] ?? [0, 0, 0];
      // Deterministic pseudo-random spread based on row ID hash
      const hash = Array.from(row.id).reduce((h, c) => ((h << 5) - h + c.charCodeAt(0)) | 0, 0);
      const spread = 8;
      const x = center[0] + ((hash & 0xFF) / 128 - 1) * spread;
      const y = center[1] + (((hash >> 8) & 0xFF) / 128 - 1) * spread;
      const z = center[2] + (((hash >> 16) & 0xFF) / 128 - 1) * spread;
      ...
      return {
        ...
        label: row.textPreview?.substring(0, 20) ?? row.id,
      };
    });
