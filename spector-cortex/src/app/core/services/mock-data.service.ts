...
export class MockDataService implements OnDestroy {
  ...
  constructor() {
    // Reactively start or stop simulation based on central useMockData signal
    effect(() => {
      const mockActive = this.state.useMockData();
      if (mockActive) {
        this.start();
      } else {
        this.stop();
  ...
  start(): void {
    ...
    // GPU kernel events every 500-1000ms (throttled to prevent OOM)
