 ...
 */
package com.spectrayan.spector.memory.kernel.engram;
...
import com.spectrayan.spector.memory.kernel.engram.compat.LegacyEncodingHeaderReader;
...
class EncodingHeaderProvenanceRoundTripTest {
    ...
    void consolidationFlagsSimulatedSurvivesRoundTrip() {
        try (Arena arena = Arena.ofConfined()) {
            ...
            assertThat(read.consolidationFlags()).isEqualTo(EncodingHeaderFields.FLAG_SIMULATED);
            assertThat(EncodingHeaderFields.isSimulated(read.consolidationFlags())).isTrue();
    ...
    void legacyV1HeaderDecodesCorrectly() {
        try (Arena arena = Arena.ofConfined()) {
            ...
            // Write raw V1 bytes
            segment.set(ValueLayout.JAVA_BYTE, EncodingHeaderFields.OFFSET_HEADER_VERSION, (byte) 1);
            segment.set(ValueLayout.JAVA_BYTE, EncodingHeaderFields.OFFSET_FLAGS, procFlags);
            segment.set(ValueLayout.JAVA_BYTE, EncodingHeaderFields.OFFSET_VALENCE, (byte) 30);
            segment.set(ValueLayout.JAVA_BYTE, EncodingHeaderFields.OFFSET_AROUSAL, (byte) 150);
            segment.set(ValueLayout.JAVA_FLOAT, EncodingHeaderFields.OFFSET_IMPORTANCE, 0.85f);
            segment.set(ValueLayout.JAVA_LONG, EncodingHeaderFields.OFFSET_TIMESTAMP, now);
            segment.set(ValueLayout.JAVA_INT, EncodingHeaderFields.OFFSET_AGENT_RECALL_COUNT, 5);
            segment.set(ValueLayout.JAVA_FLOAT, EncodingHeaderFields.OFFSET_EXACT_NORM, 1.12f);
            segment.set(ValueLayout.JAVA_LONG, EncodingHeaderFields.OFFSET_SYNAPTIC_TAGS, 0xCAFEBABE1234L);
            segment.set(ValueLayout.JAVA_SHORT, EncodingHeaderFields.OFFSET_CENTROID_ID, (short) 7);
            segment.set(ValueLayout.JAVA_BYTE, EncodingHeaderFields.OFFSET_CONSOLIDATION_FLAGS, EncodingHeaderFields.FLAG_SIMULATED);
            segment.set(ValueLayout.JAVA_BYTE, EncodingHeaderFields.OFFSET_ENCODING_PROFILE, (byte) 2);
            segment.set(ValueLayout.JAVA_FLOAT, EncodingHeaderFields.OFFSET_STORAGE_STRENGTH, 2.75f);
            segment.set(ValueLayout.JAVA_INT, EncodingHeaderFields.OFFSET_SPECTOR_RECALL_COUNT, 8);
            segment.set(ValueLayout.JAVA_BYTE, EncodingHeaderFields.OFFSET_ENCODING_ALPHA, (byte) 64);
            segment.set(ValueLayout.JAVA_BYTE, EncodingHeaderFields.OFFSET_ENCODING_BETA, (byte) 128);
            segment.set(ValueLayout.JAVA_SHORT, EncodingHeaderFields.OFFSET_SOUL_VERSION, (short) 4);
            segment.set(ValueLayout.JAVA_LONG, EncodingHeaderFields.OFFSET_LAST_AUTO_LTP, now - 1000L);
            segment.set(ValueLayout.JAVA_FLOAT, EncodingHeaderFields.OFFSET_ENCODING_SURPRISE, 1.414f);
            segment.set(ValueLayout.JAVA_BYTE, EncodingHeaderFields.OFFSET_LAST_RECALL_PROFILE, (byte) 1);
