...
/**
 * Performance benchmark tests verifying the optimizations P1â€“P12.
 *
...
class PerformanceBenchmarkTest {
    ...
    @Test
    @Order(1)
    @DisplayName("P1: MemoryIndex.findIdByOffset â€” O(1) reverse lookup at 50K entries")
    void p1_reverseIndexIsConstantTime() {
        ...
        System.out.printf("P1: 10K lookups in %,d Âµs (avg %.0f ns/lookup, found=%d)%n",
                elapsed / 1000, avgNs, found);

        // O(1) should be < 1Âµs per lookup (vs ~50Âµs for O(n) at 50K)
        assertThat(avgNs).as("O(1) reverse lookup should be under 1Âµs").isLessThan(1_000);
    ...
    @Test
    @Order(2)
    @DisplayName("P8: ScoredRecord carries CognitiveHeader â€” no re-read needed")
    ...
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    // P3: SIMD Euclidean â€” benchmarks quantized distance
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

    @Test
    @Order(3)
    @DisplayName("P3: SIMD Euclidean distance â€” 768-dim Ã— 10K vectors under 150ms")
    void p3_simdEuclideanDistance768Dim() {
        ...
        try (Arena arena = Arena.ofConfined()) {
            ...
            System.out.printf("P3: 10K Ã— 768-dim L2 in %,d ms (avg %.1f Âµs/vector, checksum=%.2f)%n",
                    elapsed / 1_000_000, avgUs, totalDist);

            // 10K Ã— 768-dim should complete in < 150ms with SIMD (with headroom for slower/virtualized test runners)
            assertThat(elapsed / 1_000_000).as("SIMD L2 10KÃ—768d should be under 150ms").isLessThan(150);
        }
    }
    ...
    @Test
    @Order(4)
    @DisplayName("P7: Batch habituation penalty â€” 1K IDs under 1ms")
    void p7_batchHabituationPenalty() {
        ...
        System.out.printf("P7: Batch 1K penalties in %,d Âµs%n", elapsed / 1000);
    ...
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    // P12: TierRouter.totalCount â€” direct sum
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

    @Test
    @Order(5)
    @DisplayName("P12: TierRouter.totalCount â€” 100K calls under 10ms (no Stream)")
    void p12_totalCountDirectSum() {
        ...
        try {
            ...
            System.out.printf("P12: 100K totalCount() in %,d Âµs (sum=%d)%n",
                    elapsed / 1000, sum);
    ...
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    // CognitiveScorer â€” 6-phase pipeline timing at scale
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

    @Test
    @Order(6)
    @DisplayName("CognitiveScorer: 10K records Ã— 128-dim â€” full 6-phase scoring under 20ms")
    void cognitiveScorer_fullPipelineTiming() {
        ...
        try (Arena arena = Arena.ofConfined()) {
            ...
            System.out.printf("CognitiveScorer: %d records Ã— %d-dim in %,d Âµs â†’ %d results%n",
                    count, dims, elapsed / 1000, results.size());

            assertThat(results).hasSize(10);
            assertThat(results.getFirst().header()).isNotNull(); // P8: header present
            assertThat(elapsed / 1_000_000).as("10K Ã— 128d scoring < 20ms").isLessThan(20);
        }
    }
