...
class SourceModalityFlagsTest {
    ...
    void modalityRoundTrip(SourceModality modality) {
        byte flags = EncodingHeaderFields.withSourceModality((byte) 0, modality.ordinal());
        int readBack = EncodingHeaderFields.sourceModalityOrdinal(flags);
    ...
    void modalityDoesNotClobberTombstone() {
        byte flags = EncodingHeaderFields.FLAG_TOMBSTONE; // bit 0 set
        flags = EncodingHeaderFields.withSourceModality(flags, SourceModality.IMAGE.ordinal());

        assertTrue(EncodingHeaderFields.isTombstoned(flags),
    ...
    void modalityDoesNotClobberMemoryType() {
        ...
        byte flags = EncodingHeaderFields.withMemoryType((byte) 0, 3);
        int originalType = EncodingHeaderFields.memoryTypeOrdinal(flags);

        // Now set modality to VIDEO (ordinal 3, bits 6-7)
        flags = EncodingHeaderFields.withSourceModality(flags, SourceModality.VIDEO.ordinal());

        assertEquals(originalType, EncodingHeaderFields.memoryTypeOrdinal(flags),
    ...
    void modalityDoesNotClobberConsolidated() {
        byte flags = EncodingHeaderFields.FLAG_CONSOLIDATED;
        flags = EncodingHeaderFields.withSourceModality(flags, SourceModality.AUDIO.ordinal());

        assertTrue(EncodingHeaderFields.isConsolidated(flags),
    ...
    void modalityDoesNotClobberPinned() {
        byte flags = EncodingHeaderFields.FLAG_PINNED;
        flags = EncodingHeaderFields.withSourceModality(flags, SourceModality.IMAGE.ordinal());

        assertTrue(EncodingHeaderFields.isPinned(flags),
    ...
    void modalityDoesNotClobberResolved() {
        ...
        assertTrue(EncodingHeaderFields.isResolved(flags),
    ...
    void allFlagsCoexist() {
        byte flags = (byte) 0;
        flags = (byte) (flags | EncodingHeaderFields.FLAG_TOMBSTONE);    // bit 0
        flags = EncodingHeaderFields.withMemoryType(flags, 3);            // bits 1-2 = PROCEDURAL (0b11)
        flags = (byte) (flags | EncodingHeaderFields.FLAG_CONSOLIDATED); // bit 3
        flags = (byte) (flags | EncodingHeaderFields.FLAG_PINNED);       // bit 4
        flags = (byte) (flags | EncodingHeaderFields.FLAG_RESOLVED);     // bit 5
        flags = EncodingHeaderFields.withSourceModality(flags, SourceModality.VIDEO.ordinal()); // bits 6-7

        assertTrue(EncodingHeaderFields.isTombstoned(flags));
        assertEquals(3, EncodingHeaderFields.memoryTypeOrdinal(flags));
        assertTrue(EncodingHeaderFields.isConsolidated(flags));
        assertTrue(EncodingHeaderFields.isPinned(flags));
        assertTrue(EncodingHeaderFields.isResolved(flags));
        assertEquals(SourceModality.VIDEO,
    ...
    void zeroFlagsDefaultToText() {
        assertEquals(SourceModality.TEXT,
                SourceModality.fromOrdinal(EncodingHeaderFields.sourceModalityOrdinal((byte) 0)),
    ...
    void memoryTypeOnlyFlagsDefaultToText() {
        ...
        byte flags = EncodingHeaderFields.withMemoryType((byte) 0, 1); // EPISODIC
        assertEquals(SourceModality.TEXT,
                SourceModality.fromOrdinal(EncodingHeaderFields.sourceModalityOrdinal(flags)),
    ...
    void overwriteModality() {
        ...
        assertEquals(SourceModality.AUDIO,
                SourceModality.fromOrdinal(EncodingHeaderFields.sourceModalityOrdinal(flags)));
        assertEquals(1, EncodingHeaderFields.memoryTypeOrdinal(flags), "Memory type should survive");
        assertTrue(EncodingHeaderFields.isPinned(flags), "Pinned flag should survive");
    }

    @Test
    @DisplayName("Overwriting modality back to TEXT zeroes bits 6-7")
    void overwriteModalityBackToText() {
        byte flags = EncodingHeaderFields.withSourceModality((byte) 0, SourceModality.VIDEO.ordinal());
        assertEquals(SourceModality.VIDEO,
                SourceModality.fromOrdinal(EncodingHeaderFields.sourceModalityOrdinal(flags)));

        flags = EncodingHeaderFields.withSourceModality(flags, SourceModality.TEXT.ordinal());
        assertEquals(SourceModality.TEXT,
                SourceModality.fromOrdinal(EncodingHeaderFields.sourceModalityOrdinal(flags)));
