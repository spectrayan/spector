 ...
 */
public final class LegacyV3Layout {

    private LegacyV3Layout() {}
    ...
    public static Path workingMem(Path basePath) {
        return StorageLayout.runtimeDir(basePath).resolve(FILE_WORKING);
    }

    public static Path coactivationTracker(Path basePath) {
        return StorageLayout.runtimeDir(basePath).resolve(FILE_COACTIVATION);
    }

    public static Path checkpointMeta(Path basePath) {
        return StorageLayout.runtimeDir(basePath).resolve(FILE_CHECKPOINT_META);
    }

    public static Path indexMidxRuntime(Path basePath) {
        return StorageLayout.runtimeDir(basePath).resolve(FILE_INDEX);
    }

    public static Path hebbianGraphRuntime(Path basePath) {
        return StorageLayout.runtimeDir(basePath).resolve(FILE_HEBBIAN);
    }

    public static Path temporalChainRuntime(Path basePath) {
        return StorageLayout.runtimeDir(basePath).resolve(FILE_TEMPORAL);
    }

    public static Path temporalFactsRuntime(Path basePath) {
        return StorageLayout.runtimeDir(basePath).resolve(FILE_TEMPORAL_FACTS);
    }

    public static Path hyperEntityGraphRuntime(Path basePath) {
        return StorageLayout.runtimeDir(basePath).resolve(FILE_HYPERGRAPH);
    }

    public static Path entityDirectoryRuntime(Path basePath) {
        return StorageLayout.runtimeDir(basePath).resolve(FILE_ENTITY_DIRECTORY);
    }

    public static Path entityTypesRuntime(Path basePath) {
        return StorageLayout.runtimeDir(basePath).resolve(FILE_ENTITY_TYPES);
    }

    public static Path relationTypesRuntime(Path basePath) {
        return StorageLayout.runtimeDir(basePath).resolve(FILE_RELATION_TYPES);
    }

    public static Path bm25BidxRuntime(Path basePath) {
        return StorageLayout.runtimeDir(basePath).resolve(FILE_BM25);
    }
