 ...
 *       new RememberHints(0.8f, 0.6f, 0.3f),  // I=0.8, C=0.6, U=0.3
 *       "database", "deadlock");
 *
 *   // With emotional context:
 *   memory.remember("mem-456", "Critical production outage!",
 *       MemoryType.EPISODIC, MemorySource.OBSERVED,
 *       new RememberHints(1.0f, 0.9f, 1.0f, (byte) -100, (byte) 200),
 *       "incident", "outage");
 * }</pre>
 *
 * <h3>Clamping</h3>
...
public record RememberHints(float interest, float challenge, float urgency,
     ...
     */
    public RememberHints(float interest, float challenge, float urgency) {
        this(interest, challenge, urgency, (byte) 0, (byte) 0);
    }

    /** Empty hints — triggers novelty-only importance computation. */
    public static final RememberHints NONE = new RememberHints(0f, 0f, 0f);
