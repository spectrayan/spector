 ...
 */
public class EntityDirectory extends AbstractGraphMemory<EntityDirectoryLayout> {
    ...
    private volatile PayloadEncryptor dataEncryptor;
    ...
    protected EntityDirectory(com.spectrayan.spector.kernel.bundle.RegionRef entityDirRef,
                            com.spectrayan.spector.kernel.bundle.RegionRef entityNamesRef,
                            int entityCapacity, TypeRegistryMemory entityTypeRegistry,
                            Path bundlePath, boolean isNew) {
    ...
    private void ensureAdjSegmentCapacity(int requiredEntries) {
        ...
        if (fileBacked) {
            ...
            if (requiredEntries <= adjSegmentCapacity) return;
            throw new SpectorMemoryTierFullException("EntityDirectory adjacency segment exhausted", requiredEntries);
        }
    ...
    public ConcurrentHashMap<String, Integer> nameIndexInternal() {
        return nameIndex;
    }
    ...
    public PayloadEncryptor dataEncryptor() {
    ...
    public void save(Path filePath, PayloadEncryptor encryptor) {
        if (bundleManaged) {
            long stamp = lock.readLock();
            try {
    ...
    public static EntityDirectory load(Path filePath, int defaultEntityCap,
                                       TypeRegistryMemory entityTypeRegistry, PayloadEncryptor encryptor) {
