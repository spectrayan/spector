...
import static com.spectrayan.spector.kernel.layout.HebbianLayout.DATA_START;
import static com.spectrayan.spector.kernel.layout.HebbianLayout.EDGE_BYTES;
import static com.spectrayan.spector.kernel.layout.HebbianLayout.EDGE_OFF_BRIDGE_SCORE;
import static com.spectrayan.spector.kernel.layout.HebbianLayout.EDGE_OFF_EDGE_FLAGS;
import static com.spectrayan.spector.kernel.layout.HebbianLayout.EDGE_OFF_LAST_CYCLE;
import static com.spectrayan.spector.kernel.layout.HebbianLayout.DATA_START;
import static com.spectrayan.spector.kernel.layout.HebbianLayout.EDGE_OFF_NEIGHBOR;
import static com.spectrayan.spector.kernel.layout.HebbianLayout.EDGE_OFF_WEIGHT;
import static com.spectrayan.spector.kernel.layout.HebbianLayout.SUB_OFF_CURRENT_CYCLE;
import static com.spectrayan.spector.kernel.layout.HebbianLayout.SUB_OFF_EDGE_CAPACITY;

/**
 * Compressed Sparse Row (CSR) layout for the Hebbian association graph, implementing
 * the Spector Memory Kernel {@link com.spectrayan.spector.kernel.shape.AbstractGraphMemory} specification.
 *
 * @see HebbianGraphBase
 */
public final class HebbianGraphMemory extends AbstractGraphMemory<HebbianLayout> implements HebbianGraphBase {
    ...
    public static final int LEGACY_HGPH_MAGIC = 0x48475048;
    /** Interim CSR container magic ('HCSR', #432), migrated via the codec. */
    public static final int INTERIM_HCSR_MAGIC = 0x48435352;
    /** Bytes of the interim HCSR header: magic+version+capacity+edgeCap+totalEdges+cycle. */
    public static final int HCSR_HEADER_BYTES = 24;
    ...
    private volatile long sessionBoundaryMs = 300000L;
    ...
    public static HebbianGraphMemory fromRegionRef(com.spectrayan.spector.kernel.bundle.RegionRef regionRef,
                                                    int capacity, int edgeCapacity, int maxDegree,
                                                    EdgeImportance edgeImportance, Path bundlePath, boolean isNew) {
        int resolvedCap = capacity;
    ...
    private HebbianGraphMemory(com.spectrayan.spector.kernel.bundle.RegionRef regionRef,
                               int capacity, int edgeCapacity, int maxDegree,
                               EdgeImportance edgeImportance, Path bundlePath, boolean isNew) {
    ...
    public HebbianGraphMemory(int capacity) {
        this(capacity, capacity * HebbianLayout.DEFAULT_EDGE_CAPACITY_FACTOR, 20, EdgeImportance.DEFAULT);
    }
    ...
    public int decayEdges(float decayFactor, GraphHealthSink metrics) {
        graphLock.lock();
        try {
            currentCycle++;
     ...
     */
    public static void writeSmkmHeader(MemorySegment head, int capacity, int edgeCapacity,
    ...
    public static HebbianGraphMemory load(Path filePath, int defaultCapacity) {
        return load(filePath, defaultCapacity, 20, EdgeImportance.DEFAULT);
    }
     ...
     */
    public static HebbianGraphMemory fromNeighbors(HebbianNeighborProvider legacy, int maxDegree,
                                            EdgeImportance edgeImportance) {
