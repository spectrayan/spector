...
/**
 * Parallel emitter wrapping tier scans in {@code madvise} sequential hints.
 */
public final class ParallelScanEmitter implements ScanEmitter {
    ...
    public ParallelScanEmitter(List<Callable<Void>> tasks,
                               float[] queryVector, float[] mins, float[] scales,
    ...
    public void emitSlabScan(Supplier<MemorySegment> segment, IntSupplier visibleCount,
        ...
        tasks.add(() -> {
            ...
            try {
                scoreFunc.score(seg, visibleCount.getAsInt(), layout,
