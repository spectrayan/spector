...
/**
 * InsulaMemory implements the self-model region (Anterior Insular Cortex)
 * storing a single, persistent JSON self-model.
 *
 * @since 1.2.0
 */
public final class InsulaMemory implements Memory<InsularLayout>, AutoCloseable {

    private final MemoryId id;
    private final Arena arena;
    private final MemorySegment segment;
    private final com.spectrayan.spector.kernel.bundle.RegionRef regionRef;
    ...
    private InsulaMemory(MemoryId id, Arena arena, MemorySegment segment, boolean bundleManaged) {
        this.id = id;
        this.arena = arena;
    ...
    private InsulaMemory(MemoryId id, com.spectrayan.spector.kernel.bundle.RegionRef regionRef, boolean bundleManaged) {
        this.id = id;
        this.arena = null;
        this.segment = null;
    ...
    /**
     * Creates an InsulaMemory region from a bundle RegionRef.
     */
    public static InsulaMemory fromRegionRef(com.spectrayan.spector.kernel.bundle.RegionRef regionRef, boolean isNew) {
        ...
        return new InsulaMemory(memoryId, regionRef, true);
    }

    /**
     * Creates an InsulaMemory region from an existing bundle slice.
     */
    public static InsulaMemory fromBundle(Arena arena, MemorySegment regionSlice, boolean isNew) {
        MemoryId memoryId = SystemMemoryId.INSULA.id();
        boolean effectivelyNew = isNew || !RegionPreamble.isValid(regionSlice, 0L);
        if (effectivelyNew) {
            long now = System.currentTimeMillis();
        ...
        return new InsulaMemory(memoryId, arena, regionSlice, true);
    }

    /**
     * Creates an in-memory heap-backed InsulaMemory for testing.
     */
    public static InsulaMemory heap() {
        ...
        return new InsulaMemory(SystemMemoryId.INSULA.id(), arena, heapSeg, false);
    }
