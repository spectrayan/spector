 ...
 *   bit 6: purged (payload physically zeroed — irreversible, distinct from tombstone)
 *   bit 7: dreamed (generated during sleep cycle)
 * </pre>
 *
 * <h3>Flag bit space — exhausted</h3>
 * <p>Both flag bytes are now fully allocated: byte 1 is {@code 0xFF} and byte 40 is {@code 0xFF} once
 * {@link #FLAG_PURGED} claims bit 6. There is <b>no free flag bit left in the engram header.</b> The next
 * state to need one must widen a field or claim part of the reserved block at
 * {@link #OFFSET_V2_RESERVED} (bytes 52-63), which is a format change requiring a schema version bump and a
 * migration — not a drop-in addition. Recorded here so that arrives as a decision rather than a surprise.</p>
 *
...
public final class EncodingHeaderFields {
    ...
    /**
     * Bit 6: Record has been <b>purged</b> — its vector payload has been physically overwritten with zeros
     * and its graph edges dropped. Irreversible.
     *
     * <p>This is strictly stronger than {@link #FLAG_TOMBSTONE} and the two are orthogonal: a tombstoned
     * record is hidden but its bytes remain on disk and in backups, whereas a purged record's payload is
     * gone. Purge sets both flags; readers that only need "do not return this" can keep checking
     * {@code FLAG_TOMBSTONE}, while readers that must not interpret the payload bytes — anything that
     * dequantizes a vector, scores a similarity, or reports content — must check {@code FLAG_PURGED},
     * because a zeroed payload is a valid-looking all-zero vector, not an absence.</p>
     *
     * <p>Lives in the consolidation-flags byte (40) rather than the main flags byte (1) because byte 1 is
     * fully allocated. This is the last free bit in byte 40 — see the class javadoc.</p>
     */
    public static final byte FLAG_PURGED = 0x40;
    ...
    public static final byte PROFILE_ORDINAL_SOUL_DERIVED = 13;
     ...
     */
    public static com.spectrayan.spector.kernel.api.MemoryType memoryTypeOf(byte flags) {
        int ord = memoryTypeOrdinal(flags);
        var values = com.spectrayan.spector.kernel.api.MemoryType.values();
        return (ord >= 0 && ord < values.length) ? values[ord] : com.spectrayan.spector.kernel.api.MemoryType.SEMANTIC;
    }
     ...
     *
     * @see com.spectrayan.spector.kernel.api.SourceModality
     */
    ...
    /**
     * Checks if the purged flag is set in the consolidation &amp; governance flags byte.
     *
     * <p>When {@code true} the record's payload bytes have been zeroed. Callers must not dequantize,
     * score, or report the vector: an all-zero payload reads as a legitimate zero vector, so treating it
     * as data silently fabricates a similarity of zero rather than reporting absence.</p>
     *
     * @see #FLAG_PURGED
     */
    public static boolean isPurged(byte consolidationFlags) {
        return (consolidationFlags & FLAG_PURGED) != 0;
    }

    /**
     * Sets or clears the purged flag on the given consolidation &amp; governance flags byte.
     *
     * <p>Clearing is provided for header round-trip fidelity only. Purge is irreversible in the data:
     * clearing the flag does not restore the zeroed payload, it only lies about it.</p>
     */
    public static byte withPurged(byte flags, boolean purged) {
        return (byte) (purged ? (flags | FLAG_PURGED) : (flags & ~FLAG_PURGED));
    }
    ...
    public static byte soulDerivedEncodingProfile() {
        return (byte) (ENCODING_FLAG_SOUL_DERIVED
                | (PROFILE_ORDINAL_SOUL_DERIVED & ENCODING_PROFILE_MASK));
    }
     ...
     */
    public static com.spectrayan.spector.kernel.api.EngramSource sourceOf(byte code) {
        return com.spectrayan.spector.kernel.api.EngramSource.fromCode(code);
    }
}
