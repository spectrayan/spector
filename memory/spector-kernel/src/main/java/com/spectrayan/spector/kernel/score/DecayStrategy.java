 ...
 * generated by {@link com.spectrayan.spector.config.properties.DecayProperties#computeBuckets}.</p>
 *
...
public final class DecayStrategy {
     ...
     */
        ...
        float[] b = new float[midpointHours.length];
        double a = Math.pow(midpointHours[0], d);
        for (int i = 0; i < midpointHours.length; i++) {
            double ret = a * Math.pow(midpointHours[i], -d);
            b[i] = (float) Math.max(floor, Math.min(1.0, ret));
        }
        b[0] = 1.0f;
        return b;
    }
