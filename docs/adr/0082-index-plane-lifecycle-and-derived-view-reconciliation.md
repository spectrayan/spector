...
- **Generation-stamped caches only.** A persisted reverse index or lexical snapshot is valid iff its generation matches the source (adj HWM + entityCount, analyzer/model id). Mismatch ⇒ discard and rebuild.
...
`RegionId` change when the cache ships:

- add `ENTITY_REVERSE_INDEX(27)` and `SPLADE(28)` (SPLADE may ship first);
- grow `LOOKUP` from 27 to at least 32;
...
- Default mode is incremental and budgeted (`spector.memory.indexes.reconcile.max-repairs-per-cycle`). Full rebuild is an explicit flag.
- Reads use existing graph read locks; never hold the entity write lock for the whole scan.
...
2. **Phase 1 — expensive derived persist**
   - Add `RegionId.SPLADE(28)` (grow `LOOKUP`), `SpladeIndex.saveToRegion` / `loadFromRegion`, `MemorySpladeIndex.persistToBundle` / `loadFromBundle`.
   - Stamp BM25 and SPLADE snapshots with analyzer / model generation.
   - Hook both into `CheckpointEngine` after graph saves.
   - Fulfils #428 for the lexical pair (`AbstractMemoryIndex` as a shared base is allowed but not required to land persist).
...
4. **Phase 3 — optional reverse cache**
   - Land `RegionId.ENTITY_REVERSE_INDEX(27)` only if Phase 0 tests show `rebuildReverseIndex` exceeds the cold-start budget at target scale.
   - Generation must equal `adjHighWaterMark` + `entityCount`. Mismatch discards the region.
