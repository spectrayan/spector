...
To bridge this gap, Spector's **Recall Pathway** combines BM25 with **SPLADE Learned Sparse Retrieval** (which expands documents with neural synonyms) and **Dense Vector Similarity**, fusing their scores using RRF before performing final graph expansions.
