...
When top-down, goal-directed retrieval is weak, **bottom-up, associative recall** can compensate — memories surface via association chains rather than directed search.

A tangential mention triggers a cascade of related memories even when the original target is unreachable. The [STDP infrastructure](../memory/association-graph.md#layer-1-hebbian-association-graph) makes this possible — directed causal edges encode "thinking about A leads to thinking about B."
